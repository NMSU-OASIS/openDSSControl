...
for multiplier in [1, 1.1, .9, .1]:
    ...
    for building in building_list:
        ...
        plt.grid()
        plt.title('Count of overvolts at each 5 kW step of solar (' + building.name + ').')
        plt.ylabel('Overvolt count')
        plt.savefig(path + 'Over_volts_' + building.name + '.png')
        pandas.DataFrame(overvoltage).to_csv('Over_volts_' + building.name + str(multiplier) + '.csv', header=False,
                                              index=False)
        plt.close()
        plt.figure()
        plt.scatter(range(start, start + steps * increment, increment), undervoltage)
        plt.grid()
        plt.title('Count of undervolts at each 5 kW step of solar (' + building.name + ').')
        plt.ylabel('Undervolt count')
        plt.savefig(path + 'Under_volts_' + building.name + '.png')
        pandas.DataFrame(undervoltage).to_csv('Under_volts_' + building.name + str(multiplier) + '.csv', header=False,
                                              index=False)
        plt.close()
        plt.figure()
        plt.scatter(range(start, start + steps * increment, increment), overload)
        plt.grid()
        plt.title('Count of overloads at each 5 kW step of solar (' + building.name + ').\nExcludes Line.L9')
        plt.ylabel('Overload count')
        plt.savefig(path + 'Overloads_' + building.name + '.png')
        pandas.DataFrame(overload).to_csv('Overloads_' + building.name + str(multiplier) + '.csv', header=False,
                                              index=False)
        plt.close()
