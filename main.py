...
if __name__ == '__main__':
    ...
    for solarDat in ['maximum.csv', 'minimum.csv', 'mean.csv', 'median.csv']:
        ...
        for building in building_list:
            ...
            #             'Voltage Means'])
            plt.savefig(path + '\\output' + solarDat[0:len(solarDat)-4] + '.png')
            fig = plt.figure()
            plt.title('Substation Power vs KW solar penetration (steps of 25)')
            plt.xlabel('kW penetration')
            plt.boxplot(p)
            plt.savefig(path + '\\outputp' + solarDat[0:len(solarDat)-4] + '.png')
            fig = plt.figure()
            plt.title('Substation Imaginary Power vs KW solar penetration (steps of 25)')
            plt.xlabel('kW penetration')
            plt.boxplot(q)
            plt.savefig(path + '\\outputq' + solarDat[0:len(solarDat) - 4] + '.png')
            # plt.show()
